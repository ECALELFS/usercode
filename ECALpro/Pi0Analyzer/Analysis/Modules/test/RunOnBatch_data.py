...
while (len(inputlist_v) > 0):
    ...
    printFillCfg(pwd, fill_cfg_f, str(ijob), workdir, OnlyContCorr, str(nInter), useES, str(cuts4s9) )
    ...
    fill_cfg_f.write("    )\n")
    fill_cfg_f.write(")\n")
    printFillCfg_2(pwd, fill_cfg_f, str(ijob), workdir, OnlyContCorr, str(nInter), useES, str(cuts4s9) )
    fill_cfg_f.close()
