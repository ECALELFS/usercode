...
//*************************************************************************************************
void plotRegressionPerformanceComparison(string applyingFilename, string targetFilename1, string targetFilename2, string targetFilename3, string targetFilename4, string versionOption1, string versionOption2, string versionOption3, string versionOption4, string outDirectoryName, Int_t Option) {  

  // Setting up graphic style
  gStyle->SetOptStat(0);

  string targetFilenames_array[] = {targetFilename1, targetFilename2, targetFilename3, targetFilename4};
  string versionOptions_array[] = {versionOption1, versionOption2, versionOption3, versionOption4};
  vector<string> targetFilenames(targetFilenames_array, targetFilenames_array+4);
  vector<string> versionOptions(versionOptions_array, versionOptions_array+4);
  int nFiles = 4;
  ...
  for (int i = 0; i < nFiles; i++) {
    ...
    if (Option == 1)  binNameStr = "Electron energy ratio, 0.8 < Eta < 1.479, 7 < Pt < 10";
    if (Option == 2)  binNameStr = "Electron energy ratio, Eta > 1.479, 7 < Pt < 10";
    if (Option == 3)  binNameStr = "Electron energy ratio, Eta < 0.8, 10 < Pt < 20";
    if (Option == 4)  binNameStr = "Electron energy ratio, 0.8 < Eta < 1.479, 10 < Pt < 20";
    if (Option == 5)  binNameStr = "Electron energy ratio, Eta > 1.479, 10 < Pt < 20";
    if (Option == 6)  binNameStr = "Electron energy ratio, Eta < 0.8, Pt > 20";
    if (Option == 7)  binNameStr = "Electron energy ratio, 0.8 < Eta < 1.479, Pt > 20";
    if (Option == 8)  binNameStr = "Electron energy ratio, Eta > 1.479, Pt > 20";
    ...
    // Histograms
    //==============================================================================================================  
    EleEnergyOverGenEnergy = new TH1F("EleEnergyOverGenEnergy", (binNameStr + "; Electron Energy / Generated Energy ; Number of Events ").c_str(),  200, 0, 2);
    TH1F *EleRegressionEnergyOverGenEnergy = new TH1F(("EleRegressionEnergyOverGenEnergy_" + versionOption).c_str(), "; Electron Regression Energy / Generated Energy ; Number of Events ",  200, 0, 2);
    ...
    if (versionOption == "V00" || versionOption == "V01") {
      //*****************************************************************************************
      //Load Regression
      //*****************************************************************************************
	...
	else if (fabs(RealEleTree.fEleEta) < 1.479) subdet = 1;
	else subdet = 2;
	...
	// Regression energy
	Double_t Energy;
	if (RealEleTree.fEleEta < 1.479) {
	  Energy = fEleRegressionTargetEB * RealEleTree.fSCRawEnergy;
	} else if (RealEleTree.fEleEta > 1.479) {
	...
	if (RealEleTree.fElePt <= 15.0) {						// For pt < 15, use pt instead of SCRawEnergy
	  if (RealEleTree.fEleEta < 1.479) {
	    Energy = fEleRegressionTargetEB_lowPt * RealEleTree.fElePt;
	  } else if (RealEleTree.fEleEta > 1.479) {
	    Energy = fEleRegressionTargetEE_lowPt * ( RealEleTree.fElePt * (1 + RealEleTree.fElePreShowerOverRaw ) );
	  }
	}
  ...
  EleRegressionEnergyOverGenEnergy_hists[3]->SetLineColor(kViolet);
  ...
  EleRegressionEnergyOverGenEnergy_hists[3]->SetLineWidth(2);
